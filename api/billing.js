// api/billing.js (merged offer + wallet)
...
async function offerHandler(req, res) {
...
export async function wallet_topup_demo_handler(req, res) {
  try {
    ...
    const { error: we } = await sb.from('wallets')
      .upsert([{ user_id, balance_uzs: nextBal, updated_at: nowIso() }], { onConflict:'user_id' });
    if (we) throw we;

    const { data: tx, error: te } = await sb.from('wallet_transactions')
...
export default async function handler(req, res) {
  ...
  if (key === 'offer') return offerHandler(req, res);
  if (key === 'wallet') return walletHandler(req, res);
  // fallback by path
  const p = String(req.url||'');
  if (p.includes('/offer')) return offerHandler(req, res);
  if (p.includes('/wallet')) return walletHandler(req, res);
  res.statusCode = 404;
  return res.end(JSON.stringify({ error: 'Unknown billing route', key }));
}
