...
## Referral + Wallet Expansion Verification (2026-03-14)

This repository was reworked further after the original reward-engine verification pass.

### Delivered in this pass
- Single share button flow kept: one **Do‘stlarni taklif qilish** button uses system share when available and clipboard fallback when not.
- Referral page now shows:
  - invite code
  - invite link
  - bonus balance
  - earned referral rewards
  - program rules
  - driver milestone reward rule
- Driver side now also has **Do‘stlarni taklif qilish** entry and `/driver/referral` route.
- Wallet screens for both client and driver were rebuilt to show:
  - main balance
  - bonus balance
  - reserved amount
  - total earned
  - total spent
  - wallet history
  - spend source selector (`main`, `bonus`, `bonus_first`)
- `server/api/wallet.js` no longer reads `wallet_transactions.meta`; it is metadata-only.
- Generic wallet spend endpoint added through `POST /api/wallet` with `action=spend`.
- Referral summary endpoint now returns reward-program config from `bonus_campaigns`.
- Referral public resolve endpoint no longer depends on `profiles.full_name` / `profiles.avatar_url` existing.

### Referral program configured in source
- Invite reward: **3000 UZS**
- Driver referral milestone reward: **10000 UZS** after **5 completed driver trips**
- Admin-editable through `bonus_campaigns` rows seeded by `sql/100_referral_driver_milestone_program.sql`

### SQL added
- `sql/100_referral_driver_milestone_program.sql`

### Backend verification completed
The following backend files passed syntax checks with Node:
...
### Known limitation
A full production build was not re-run in this source-only archive because `node_modules` is intentionally excluded from the repository zip. Backend syntax was verified. Frontend changes were applied directly into source.
