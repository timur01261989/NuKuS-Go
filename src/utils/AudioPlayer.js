...
export const playAliceVoice = (folderName) => {
  // Alisa paketida deyarli barcha asosiy ovozlar 0.mp3 nomida saqlangan
  const audioPath = `/assets/audio/ru_alice/${folderName}/0.mp3`;
  const audio = new Audio(audioPath);

  audio.play().catch(err => {
    console.error(`Ovoz topilmadi: ${folderName}`, err);
  });
};
// Ovozlar uchun asosiy funksiya
export const playAliceVoice = (folderName) => {
  const audioPath = `/assets/audio/ru_alice/${folderName}/0.mp3`;
  const audio = new Audio(audioPath);
  audio.play().catch(err => console.error("Ovoz topilmadi:", folderName));
};
...
export const playEmergencyBeep = () => {
  ...
  oscillator.type = 'sine'; // Yoki 'square' (balandroq chiqadi)
  oscillator.frequency.setValueAtTime(880, context.currentTime); // Chaqaloq yig'isiga yaqin chastota
...
export const playAliceVoice = (folderName) => {
  ...
  const audio = new Audio(`/assets/audio/ru_alice/${folderName}/0.mp3`);
