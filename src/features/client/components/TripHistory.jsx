...
export default function TripHistory({ onBack }) {
  ...
  const fetchHistory = async () => {
    ...
    if (user) {
      const { data } = await supabase.from('orders').select('*').eq('passenger_id', user.id).eq('status', 'completed').order('created_at', { ascending: false });
  ...
  return (
    <div style={{ padding: '20px', background: '#f8f9fa', minHeight: '100vh' }}>
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 25 }}>
        ...
        <Title level={4} style={{ margin: '0 0 0 15px', fontFamily: 'YangoHeadline' }}>{t.orderHistoryTitle}</Title>
      </div>
      {loading ? (
          ...
          renderItem={(trip) => (
            <Card style={{ borderRadius: 20, marginBottom: 15, border: 'none', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }} bodyStyle={{ padding: '15px' }}>
              ...
              <div style={{ marginTop: 15, paddingTop: 10, borderTop: '1px solid #f0f0f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                ...
                <Button type="default" icon={<CustomerServiceOutlined />} style={{ borderRadius: 12 }} onClick={() => { window.location.href = `/client/support/${trip.id}`; }}>{t.help}</Button>
              </div>
            </Card>
          )}
        />
