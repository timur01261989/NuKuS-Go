...
import { useTaxiOrder, money } from "./hooks/useTaxiOrder";
...
const pickupIcon = L.divIcon({
  className: "yg-pin",
  html: `<div class="yg-pin-wrap"><div class="yg-pin-top"><div class="yg-pin-yellow"><svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4Z" fill="rgba(0,0,0,.75)"/><path d="M4 22c0-4.42 3.58-8 8-8s8 3.58 8 8" stroke="rgba(0,0,0,.75)" stroke-width="2" stroke-linecap="round"/></svg></div></div><div class="yg-pin-stem"></div><div class="yg-pin-dot"></div></div>`,
  iconSize: [44, 62], iconAnchor: [22, 54],
});

const destIcon = L.divIcon({
  className: "yg-dest",
  html: `<div class="yg-dest-wrap"><div class="yg-dest-top"><div class="yg-dest-flag"><svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M6 3v18" stroke="rgba(0,0,0,.75)" stroke-width="2" stroke-linecap="round"/><path d="M6 4h11l-2 4 2 4H6" fill="rgba(0,0,0,.1)" stroke="rgba(0,0,0,.75)" stroke-width="2" stroke-linejoin="round"/></svg></div></div><div class="yg-pin-stem"></div><div class="yg-pin-dot red"></div></div>`,
  iconSize: [44, 62], iconAnchor: [22, 54],
});
...
function CenterWatcher({ onCenterChange, onMoveStart, onMoveEnd }) {
  ...
  useEffect(() => {
    if (!map) return;
    const fire = () => { const c = map.getCenter(); onCenterChange?.([c.lat, c.lng], map.getZoom()); };
    const ms = () => onMoveStart?.();
    const me = () => { onMoveEnd?.(); fire(); };
    map.on("movestart", ms); map.on("moveend", me); map.on("zoomend", me);
    fire();
    return () => { map.off("movestart", ms); map.off("moveend", me); map.off("zoomend", me); };
  }, [map, onCenterChange, onMoveStart, onMoveEnd]);
  return null;
}

function LocateMeButton({ mapRef, userLoc, setUserLoc, setPickup, bottom = 240 }) {
  return (
    <div style={{ position: "absolute", right: 16, bottom, zIndex: 800 }}>
      <Button
        shape="circle" size="large" icon={<AimOutlined style={{ fontSize: 22 }} />}
        style={{ boxShadow: "0 4px 12px rgba(0,0,0,0.15)" }}
        onClick={() => {
          const map = mapRef.current;
          if (map && userLoc) { map.flyTo(userLoc, 16, { duration: 0.6 }); return; }
          if (!navigator.geolocation) { message.error("Geolokatsiya mavjud emas"); return; }
...
export default function ClientTaxiPage() {
  ...
  const Header = (
    <div className="yg-header">
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
        <Switch checked={darkMode} onChange={(v) => setDarkMode(v)} checkedChildren="🌙" unCheckedChildren="☀️" />
      </div>
    </div>
  );
  ...
  const PickupSummaryRow = (
    <div className="yg-field">
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <Button size="small" className="yg-chip" onClick={openPickupMapEdit}>Xarita</Button>
        <Button size="small" className="yg-chip" onClick={() => setPodyezdOpen(true)}>Podyezd</Button>
      </div>
    </div>
  );
  ...
  const MainSheet = (
    <div className="yg-sheet">
      ...
      <Button className="yg-long" onClick={openDestinationSearch}>
        Qaerga borasiz? <span className="yg-long-right">›</span>
      </Button>
      ...
      <div style={{ marginTop: 10 }}>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 10 }}>
          <Button icon={<HomeOutlined />} style={{ flex: 1, borderRadius: 12 }} disabled={!homeAddr} onClick={() => applyDestinationFromAddressString(homeAddr?.address)}>Uy</Button>
          <Button icon={<BankOutlined />} style={{ flex: 1, borderRadius: 12 }} disabled={!workAddr} onClick={() => applyDestinationFromAddressString(workAddr?.address)}>Ish</Button>
        </div>
      ...
      <div className="yg-bottom-row">
        <Button className="yg-blue" type="primary" onClick={handleOrderCreate}>Buyurtma berish</Button>
  ...
  const SearchDrawer = (
    <TaxiSearchSheet
      ...
      openDestMapSelect={openDestMapEdit} // Using openDestMapEdit directly here as it was aliased to openDestMapSelect in original logic
  ...
  const DestMapSheet = (
    <DestinationPicker
      ...
      MAX_KM={MAX_KM}
      message={message}
      onConfirm={() => { setStep("route"); setSearchOpen(false); }}
    />
  );
    ...
    <div className="yg-sheet">
      <div className="yg-route-top">
        <div className="yg-route-row">
          ...
          <Button size="small" className="yg-chip" onClick={openDestinationSearch}>{dest.address ? "Podyezd" : "Xarita"}</Button>
        </div>
        <div className="yg-route-row">
          ...
          <Button size="small" className="yg-chip" onClick={changeDestination}>O‘zgartirish</Button>
        </div>
        <div style={{ marginTop: 10 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ fontSize: 12, opacity: 0.7 }}>Oraliq bekatlar</div>
            <Button size="small" icon={<PlusOutlined />} className="yg-chip" onClick={() => { setAddStopOpen(true); setStep('stop_map'); }} disabled={waypoints.length >= 3}>+</Button>
          </div>
          {waypoints.length === 0 ? (
            ...
            <div style={{ marginTop: 6 }}>
              {waypoints.map((w, idx) => (
                <div key={idx} className="yg-stop">
                  ...
                  <Button size="small" icon={<CloseOutlined />} className="yg-chip" onClick={() => setWaypoints((arr) => arr.filter((_, i) => i !== idx))} />
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 10 }}>
          <Button size="small" style={{ borderRadius: 999 }} icon={<ShareAltOutlined />} onClick={() => {
            const p = pickup?.latlng; const d = dest?.latlng;
        ...
        <div className="yg-tariffs">
          {tariffs.map((t) => (
            <button key={t.id} className={"yg-tariff " + (tariff.id === t.id ? "active" : "")} onClick={() => setTariff(t)}>
        ...
        <div style={{ marginTop: 14, background: "rgba(255,255,255,0.85)", borderRadius: 14, padding: 12 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
            ...
            <Segmented size="small" value={orderFor} options={[{ label: "O‘zimga", value: "self" }, { label: "Boshqaga", value: "other" }]} onChange={(v) => setOrderFor(v)} />
          </div>
          {orderFor === "other" && (<Input prefix={<PhoneOutlined />} value={otherPhone} onChange={(e) => setOtherPhone(e.target.value)} placeholder="U odamning telefon raqami" />)}
          <div style={{ display: "flex", gap: 10, marginTop: 10 }}>
            <Button icon={<ClockCircleOutlined />} onClick={() => setScheduleOpen(true)} style={{ flex: 1, borderRadius: 12 }}>Rejalash</Button>
          </div>
        ...
        <div className="yg-bottom-row" style={{ marginTop: 14 }}>
          <Button className="yg-yellow" type="primary" onClick={handleOrderCreate}>Buyurtma berish</Button>
  ...
  const SearchingSheet = (
    <div className="yg-sheet">
      ...
      <div className="yg-bottom-row" style={{ marginTop: 14 }}>
        <Button className="yg-gray" icon={<CloseOutlined />} onClick={handleCancel}>Safarni bekor qilish</Button>
        <Button className="yg-gray" onClick={() => message.info("Tafsilotlar keyin qo'shiladi")}>Tafsilotlar</Button>
      </div>
      <div style={{ marginTop: 14 }}>
        <AutoMarketAdsPanel title="Avto savdo e’lonlari" mode="waiting" onOpenAd={(id) => navigate(`/auto-market/ad/${id}`)} fetchAds={listMarketCars} />
      </div>
    </div>
  );
  ...
  const ComingSheet = (
    <div className="yg-sheet">
      <div style={{ fontSize: 26, fontWeight: 900 }}>{etaMin ? `~${etaMin} daq va keladi` : "Haydovchi yo'lda"}</div>
      <div className="yg-driver-card">
        <div className="yg-driver-top">
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ fontSize: 16, fontWeight: 900 }}>Haydovchi <span style={{ marginLeft: 6 }}><StarFilled style={{ color: "#faad14" }} /> {assignedDriver?.rating?.toFixed?.(2) || "4.80"}</span></div>
          </div>
      ...
      <Divider style={{ margin: "12px 0" }} />
      <AutoMarketAdsPanel title="Avto savdo e’lonlari" mode="waiting" onOpenAd={(id) => navigate(`/auto-market/ad/${id}`)} fetchAds={listMarketCars} />
      <div className="yg-cancel-link" onClick={handleCancel}>Safarni bekor qilish</div>
    </div>
  );
  ...
  const CenterPin = useMemo(() => {
    ...
    return (
      <div className="yg-pin-wrap" aria-hidden="true">
        <div className="yg-pin"><span className="material-symbols-outlined">location_on</span></div>
  ...
  const CenterPinOverlay = (
    <div className={"yg-center-pin " + (isDraggingMap ? "lift" : "")}>
      <div dangerouslySetInnerHTML={{ __html: (step === "main" || step === "search") ? pickupIcon.options.html : destIcon.options.html }} />
    </div>
  );

  const StopPickerOverlay = (
    <div style={{ position: "fixed", inset: 0, zIndex: 2500, display: step === "stop_map" && addStopOpen ? "flex" : "none", flexDirection: "column", justifyContent: "flex-end", background: "rgba(0,0,0,0.15)" }} onClick={() => { setAddStopOpen(false); setStep("route"); }}>
      <div style={{ background: "#fff", borderTopLeftRadius: 18, borderTopRightRadius: 18, padding: 14, boxShadow: "0 -8px 30px rgba(0,0,0,0.25)" }} onClick={(e) => e.stopPropagation()}>
        <div style={{ fontSize: 16, fontWeight: 900, marginBottom: 6 }}>Oraliq bekat qo‘shish</div>
        <div style={{ fontSize: 12, opacity: 0.7, marginBottom: 10 }}>Xaritani kerakli joyga olib boring, so‘ng “Qo‘shish”ni bosing.</div>
        <div className="yg-small-box" style={{ marginBottom: 12 }}>{(destAddrFromCenter || pickupAddrFromCenter) || "Manzil aniqlanmoqda..."}</div>
        <div style={{ display: "flex", gap: 10 }}>
          <Button style={{ flex: 1, borderRadius: 12 }} onClick={() => { setAddStopOpen(false); setStep("route"); }}>Bekor</Button>
          <Button type="primary" style={{ flex: 1, borderRadius: 12, background: "#000", borderColor: "#000" }} onClick={() => { addStopFromCenter(); setStep("route"); }}>Qo‘shish</Button>
        </div>
      </div>
    </div>
  );
  ...
  const PodyezdModal = (
    <Modal open={podyezdOpen} onCancel={() => setPodyezdOpen(false)} onOk={() => setPodyezdOpen(false)} okText="Saqlash" cancelText="Bekor" title="Podyezd (kirish joyi)">
      <Input value={pickup.entrance} onChange={(e) => updateEntrance(e.target.value)} placeholder="Masalan: 2" maxLength={8} />
      <div style={{ marginTop: 8, fontSize: 12, opacity: 0.7 }}>Podyezd haydovchiga aniq eshik tagiga kelish uchun ko‘rsatiladi.</div>
    </Modal>
  );

  const WishesModal = (
    <Modal open={wishesOpen} onCancel={() => setWishesOpen(false)} onOk={() => setWishesOpen(false)} okText="Tayyor" cancelText="Bekor" title="Istaklar (Pozhelaniya)">
      <div style={{ display: "grid", gap: 10 }}>
        <Checkbox checked={!!wishes.ac} onChange={(e) => setWishes((w) => ({ ...w, ac: e.target.checked }))}>❄️ Konditsioner kerak</Checkbox>
        <Checkbox checked={!!wishes.trunk} onChange={(e) => setWishes((w) => ({ ...w, trunk: e.target.checked }))}>🧳 Yukxona (bagaj) bo‘sh bo‘lsin</Checkbox>
        <Checkbox checked={!!wishes.childSeat} onChange={(e) => setWishes((w) => ({ ...w, childSeat: e.target.checked }))}>👶 Bolalar o‘rindig‘i (kreslo)</Checkbox>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 140, opacity: 0.75 }}>🚬 Chekish</div>
          <Segmented options={[{ label: "Mumkin emas", value: "no" }, { label: "Mumkin", value: "yes" }]} value={wishes.smoking} onChange={(v) => setWishes((w) => ({ ...w, smoking: v }))} />
        </div>
        <div>
          <div style={{ fontSize: 12, opacity: 0.7, marginBottom: 6 }}>📒 Izoh</div>
          <Input.TextArea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Masalan: Podyezd orqada, shlagbaum kodi: 1234..." autoSize={{ minRows: 3, maxRows: 6 }} />
        </div>
      </div>
    </Modal>
  );
  ...
  const ScheduleModal = (
    <Modal open={scheduleOpen} onCancel={() => setScheduleOpen(false)} onOk={() => setScheduleOpen(false)} okText="Tayyor" cancelText="Bekor" title="Oldindan buyurtma (Zaplanirovat)">
      <div style={{ display: "grid", gap: 10 }}>
        <Segmented options={[{ label: "Hozir", value: "now" }, { label: "Vaqtni tanlash", value: "pick" }]} value={scheduledTime ? "pick" : "now"} onChange={(v) => { if (v === "now") setScheduledTime(null); if (v === "pick" && !scheduledTime) setScheduledTime(new Date(Date.now() + 15 * 60 * 1000).toISOString()); }} />
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <ClockCircleOutlined />
          <Input type="datetime-local" value={scheduledTime ? new Date(scheduledTime).toISOString().slice(0, 16) : ""} onChange={(e) => { const v = e.target.value; if (!v) return setScheduledTime(null); try { setScheduledTime(new Date(v).toISOString()); } catch { setScheduledTime(null); } }} />
        </div>
        <div style={{ fontSize: 12, opacity: 0.7 }}>Agar vaqt tanlansa, serverga <b>scheduled_time</b> sifatida yuboriladi.</div>
      </div>
    </Modal>
  );
  ...
  const ShareModal = (
    <Modal open={shareOpen} onCancel={() => setShareOpen(false)} footer={null} title="Buyurtmani ulashish">
      <div style={{ fontSize: 12, opacity: 0.7, marginBottom: 8 }}>Havolani do‘stingizga yuboring. U sizning holatingizni ko‘ra oladi.</div>
      <Input value={shareLink} readOnly />
      <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
        <Button type="primary" onClick={async () => { try { await navigator.clipboard.writeText(shareLink); message.success("Nusxa olindi"); } catch { message.info("Nusxa olish uchun brauzer ruxsat bermadi"); } }}>Nusxa olish</Button>
  ...
  const Styles = (
    <style>{`
      .yg-header{ position: fixed; top: 10px; left: 10px; right: 10px; z-index: 1200; display:flex; align-items:center; gap:10px; pointer-events:auto; }
      .yg-header .ant-btn{ box-shadow: 0 6px 16px rgba(0,0,0,.18); border: none; }
      .yg-sheet{ position: fixed; left: 0; right: 0; bottom: 0; z-index: 1100; background: rgba(255,255,255,.96); backdrop-filter: blur(10px); border-top-left-radius: 22px; border-top-right-radius: 22px; padding: 18px 16px 18px; box-shadow: 0 -14px 40px rgba(0,0,0,.18); max-width: 520px; margin: 0 auto; }
      ...
      .yg-long{ width: 100%; height: 56px; border-radius: 18px; font-size: 18px; font-weight: 700; display:flex; align-items:center; justify-content:center; background: #f2f2f2; border: none; }
      ...
      .yg-pin-yellow{ width: 44px; height: 44px; border-radius: 18px; background: #f6c200; box-shadow: 0 12px 26px rgba(0,0,0,.18); display:flex; align-items:center; justify-content:center; }
      .yg-dest-flag{ width: 44px; height: 44px; border-radius: 18px; background: #fff; box-shadow: 0 12px 26px rgba(0,0,0,.18); display:flex; align-items:center; justify-content:center; }
      ...
      .yg-wave{ position:absolute; left:50%; top:50%; width: 18px; height: 18px; border-radius: 50%; border: 2px solid rgba(24,144,255,.55); transform: translate(-50%,-50%); animation: yg-wave 1.4s infinite; z-index: 650; pointer-events:none; }
      .yg-wave:nth-child(2){ animation-delay: .35s; }
      .yg-wave:nth-child(3){ animation-delay: .7s; }
      @keyframes yg-wave{ 0%{ opacity:.8; transform: translate(-50%,-50%) scale(1); } 100%{ opacity:0; transform: translate(-50%,-50%) scale(7); } }
      .yg-vehicle-icon .yg-car-label{ position:absolute; left:50%; top: -28px; transform: translateX(-50%); background:#fff; border-radius: 999px; padding: 6px 10px; font-size: 12px; font-weight: 900; box-shadow: 0 10px 20px rgba(0,0,0,.18); white-space: nowrap; }
    `}</style>
  );

  const RouteLine = route?.coords && route.coords.length >= 2 ? (
    <Polyline positions={route.coords} pathOptions={{ weight: 6, opacity: 0.9 }} />
  ) : null;

  const SearchingOverlay = step === "searching" ? (
    <>
      <div className="yg-wave" /> <div className="yg-wave" /> <div className="yg-wave" />
      {dispatchLine ? <Polyline positions={dispatchLine} pathOptions={{ weight: 4, opacity: 0.65, dashArray: "8 10" }} /> : null}
      {nearCars.map((c, idx) => (
        <VehicleMarker key={c.id} position={[c.lat, c.lng]} bearing={c.bearing} label={idx === dispatchIdx ? `${Math.max(1, Math.round(durationMin / 2))} daq` : undefined} color={idx === dispatchIdx ? "#f6c200" : "#ddd"} durationMs={700} />
      ))}
    </>
  ) : null;
  ...
  const DriverOverlay = step === "coming" && assignedDriver?.lat && assignedDriver?.lng ? (
    <>
      <VehicleMarker position={[assignedDriver.lat, assignedDriver.lng]} bearing={assignedDriver.bearing} label={etaMin ? `${etaMin} daq` : undefined} color="#f6c200" durationMs={800} />
      {pickup.latlng ? <Polyline positions={[[assignedDriver.lat, assignedDriver.lng], pickup.latlng]} pathOptions={{ weight: 6, opacity: 0.9 }} /> : null}
    </>
  ) : null;
  ...
  return (
    <div style={{ width: "100%", height: "100vh" }}>
      ...
      {ShareModal}
      <CenterPinOverlay />
      <CenterWatcher onCenterChange={setCenterLatLng} onMoveStart={() => setIsDraggingMap(true)} onMoveEnd={() => setIsDraggingMap(false)} />
      <LocateMeButton mapRef={mapRef} userLoc={userLoc} setUserLoc={setUserLoc} setPickup={setPickup} bottom={mapBottom} />
      ...
      <RatingModal
        ...
        onFinish={() => {
          ...
          if (earnedBonus > 0) { setTimeout(() => setBonusVisible(true), 300); }
          else { setTimeout(() => { setOrderId(null); setOrderStatus(null); setAssignedDriver(null); setStep("main"); }, 500); }
        }}
      />
      <ClientBonusWidget
        ...
        onClose={() => {
          ...
          setEarnedBonus(0);
          setTimeout(() => { setOrderId(null); setOrderStatus(null); setAssignedDriver(null); setStep("main"); }, 500);
        }}
      />
    </div>
  );
