...
import { ArrowLeftOutlined, CloseOutlined, EnvironmentOutlined, FlagOutlined, StarFilled } from "@ant-design/icons";
    ...
    >
      <div className="yg-search-top">
        <div className="yg-search-head">
          <Button type="text" icon={<ArrowLeftOutlined />} onClick={() => { setSearchOpen?.(false); setStep?.("main"); }} />
        ...
        <div className="yg-search-fields">
          <div className="yg-search-field" onClick={() => setActiveField("pickup")}>
            <div className="yg-search-ic"><EnvironmentOutlined /></div>
            <div style={{ flex: 1 }}>
              <div className="yg-search-cap">{cp("Yo‘lovchini olish nuqtasi")}</div>
              <Input value={pickupSearchText} onChange={(e) => setPickupSearchText?.(e.target.value)} onFocus={() => setActiveField("pickup")} placeholder={pickupAddress || cp("Manzil kiriting...")} bordered={false} />
            </div>
            <Button size="small" className="yg-chip" onClick={(e) => { e.stopPropagation(); openPickupMapEdit?.(); }}>{cp("Xarita")}</Button>
          </div>

          <div className="yg-search-field" onClick={() => setActiveField("dest")}>
            <div className="yg-search-ic"><FlagOutlined /></div>
            <div style={{ flex: 1 }}>
              <div className="yg-search-cap">{cp("Yakuniy manzil")}</div>
              <Input value={destSearchText} onChange={(e) => setDestSearchText?.(e.target.value)} onFocus={() => setActiveField("dest")} placeholder={cp("Qaerga borasiz?")} bordered={false} />
            </div>
            <Button size="small" className="yg-chip" onClick={(e) => { e.stopPropagation(); openDestMapSelect?.(); }}>{cp("Xarita")}</Button>
          </div>

          <div style={{ padding: "0 18px 10px", display: "flex", gap: 8 }}>
            <Tag color={activeField === "pickup" ? "blue" : "default"} style={{ margin: 0 }}>{cp("Pickup")}</Tag>
            <Tag color={activeField === "dest" ? "green" : "default"} style={{ margin: 0 }}>{cp("Destination")}</Tag>
        ...
        {listData.saved.length > 0 && (
          <>
            ...
            <List
              ...
              renderItem={(p) => (
                <List.Item className="yg-list-item" onClick={() => setDestFromSuggestion?.({ id: p.id || p.place_id, label: p.label || p.name, address: p.label || p.name, lat: Number(p.lat), lng: Number(p.lng) })}>
                  <List.Item.Meta
                    title={<div style={{ display: "flex", alignItems: "center", gap: 8 }}><StarFilled style={{ color: "#faad14" }} /><span style={{ fontWeight: 700 }}>{p.name || cp("Manzil")}</span></div>}
