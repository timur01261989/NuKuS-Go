  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f6f8" }}>
      ...
      <div style={{
        ...
        <Button type="text" onClick={open} icon={<MenuOutlined style={{ color: "white", fontSize: 20 }} />} />
      ...
      <div style={{ padding: 14, maxWidth: 560, margin: "0 auto" }}>
        ...
        <Space direction="vertical" size={12} style={{ width: "100%" }}>
          ...
          <Card hoverable style={{ borderRadius: 16 }} onClick={() => navigate("/client/inter-provincial")}>
            <Space>
              <EnvironmentOutlined />
              <div>
          ...
          <Card hoverable style={{ borderRadius: 16 }} onClick={() => navigate("/client/inter-district")}>
            <Space>
              <RocketOutlined />
              <div>
          ...
          <Button
            ...
            block
            icon={<ShopOutlined />}
            style={{ height: 50, borderRadius: 14, background: "#000", borderColor: "#000", fontWeight: 800 }}
        ...
        <div style={{ marginTop: 14 }}>
          <Title level={5} style={{ marginBottom: 10 }}>{t?.newCars || "Yangi qo‘shilgan mashinalar"}</Title>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {(cars.length ? cars : Array.from({ length: 4 }).map((_, i) => ({ id: "p"+i }))).map((c, i) => (
              <Card
                key={c.id || i}
                hoverable
                ...
                onClick={() => navigate("/auto-market")}
              >
                <div style={{ height: 90, borderRadius: 12, background: "#f0f0f0" }} />
                <div style={{ marginTop: 8, fontWeight: 800 }}>{c.title || c.model || `Mashina ${i+1}`}</div>
      ...
      {/* Sidebar */}
      <Drawer placement="left" open={drawerOpen} onClose={close} width={300} bodyStyle={{ padding: 0 }}>
        ...
        <div style={{ padding: 12, position: "relative", minHeight: "calc(100vh - 80px)" }}>
          <Button block icon={<UserOutlined />} style={{ height: 44, borderRadius: 12, textAlign: "left" }} onClick={() => { close(); navigate("/driver-mode"); }}>
            {t?.workAsDriver || "Haydovchi bo‘lib ishlash"}
          </Button>

          <Divider style={{ margin: "12px 0" }} />

          <Button block icon={<EnvironmentOutlined />} style={{ height: 44, borderRadius: 12, textAlign: "left" }} onClick={() => { close(); navigate("/addresses"); }}>
            {t?.myAddresses || "Mening manzillarim"}
          </Button>

          <Button block icon={<SettingOutlined />} style={{ height: 44, borderRadius: 12, textAlign: "left", marginTop: 8 }} onClick={() => { close(); navigate("/settings"); }}>
            {t?.settings || "Sozlamalar"}
          </Button>

          <Button block icon={<HistoryOutlined />} style={{ height: 44, borderRadius: 12, textAlign: "left", marginTop: 8 }} onClick={() => { close(); navigate("/orders"); }}>
            {t?.orderHistory || "Buyurtmalar tarixi"}
          </Button>

          <div style={{ position: "absolute", left: 12, right: 12, bottom: 12 }}>
            <Button block icon={<CustomerServiceOutlined />} style={{ height: 44, borderRadius: 12, textAlign: "left" }} onClick={() => { close(); navigate("/support"); }}>
              {t?.support || "Qo‘llab-quvvatlash"}
            </Button>

            <Button danger block icon={<LogoutOutlined />} style={{ height: 44, borderRadius: 12, textAlign: "left", marginTop: 8 }} onClick={() => { close(); navigate("/logout"); }}>
              {t?.logout || "Chiqish"}
            </Button>
          </div>
        </div>
...
function AdSlot({ title }) {
  return (
    <Card style={{ borderRadius: 16 }}>
      ...
      <div style={{ height: 90, borderRadius: 12, background: "#f0f0f0", marginTop: 10 }} />
    </Card>
  );
}
