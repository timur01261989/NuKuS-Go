...
function uid() {
  return "car_" + Math.random().toString(16).slice(2) + "_" + Date.now();
}
function fmtNow() {
...
export default function PostAdForm({ onDone }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error("PostAdForm init error:", e);
        if (!mounted) return;
        setParams(null);
        setRemote(null);
      }
  ...
  const maxW = remote?.market?.post?.image_max_width ?? 1280;
  ...
  async function submit() {
    ...
    try {
      ...
      console.error("PostAdForm submit error:", e);
  ...
  return (
    <div className="space-y-3">
      <div className="text-sm text-gray-600">
        E’lon berish (test rejim: local saqlanadi)
      </div>
