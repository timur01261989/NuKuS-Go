...
export default function DriverDashboard() {
    ...
    (async () => {
      try {
        ...
        try {
          ...
          // .single() bo‘lsa va topilmasa error qaytarishi mumkin,
          // shuning uchun pErr ni throw qilmaymiz, faqat data bo‘lsa ishlatamiz.
          if (!pErr && p?.full_name) fullName = p.full_name;
          if (!pErr && p?.avatar_url) avatarUrl = p.avatar_url;
        } catch {
          // ignore
        }
  ...
  // ✅ Drawer ichidagi tugmalar bosilganda: avval drawer yopilsin, keyin navigate
  const go = (path) => {
    setDrawerOpen(false);
    // Drawer animatsiya paytida navigate bo'lsa UI "qotib" qolmasligi uchun microtask
    setTimeout(() => navigate(path), 0);
  };

  // ✅ Header tugmasida ham style/transform "stuck" bo'lmasligi uchun touch props
  const btnTouchProps = {
    onMouseDown: (e) => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg-layout)", color: "var(--text)" }}>
      <div
        style={{
          height: 56,
          display: "flex",
      ...
      <div style={{ padding: 14, maxWidth: 560, margin: "0 auto" }}>
        ...
        <Space direction="vertical" size={12} style={{ width: "100%" }}>
          ...
          <Card
            ...
            onClick={() => navigate("/driver/inter-provincial")}
            {...btnTouchProps}
          >
            <Space>
              <EnvironmentOutlined />
          ...
          <Card
            ...
            onClick={() => navigate("/driver/inter-district")}
            {...btnTouchProps}
          >
            <Space>
              <RocketOutlined />
          ...
          <Card
            ...
            onClick={() => navigate("/driver/freight")}
            {...btnTouchProps}
          >
            <Space>
              <RocketOutlined />
