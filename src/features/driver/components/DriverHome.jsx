...
// --- YORDAMCHI KOMPONENTLAR (ishlatilishidan OLDIN e'lon qilingan) ---
...
export default function DriverHome({ onLogout }) {
  ...
  // localStorage xavfsiz o'qish (SSR yoki xatolik bo'lsa null)
  const getInitialService = () => {
    try {
      return localStorage.getItem("driverActiveService") || null;
    } catch {
      return null;
    }
  };
  ...
  useEffect(() => {
    try {
      ...
      // localStorage mavjud bo'lmasa (masalan, private mode) - xatolik chiqarmaymiz
    }
  }, [selectedService]);
  ...
  const onProfileTouchMove = (e) => {
    if (touchStartY.current !== null && drawerInnerRef.current) {
      ...
      if (dy > 0) drawerInnerRef.current.style.transform = `translateY(${dy}px)`;
    }
  };
